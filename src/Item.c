    ...
    if ( item != NULL ) {
        switch ( item->tipo ) {
            ...
            case TIPO_ITEM_ESTRELA:
                ...
                break;
            case TIPO_ITEM_PLACAFIM:
                destruirItemPlacaFim( (ItemPlacaFim*) item->objeto );
                break;
            default:
                break;
        }
...
void atualizarItem( Item *item, float delta ) {
    ...
    switch ( item->tipo ) {
        ...
        case TIPO_ITEM_ESCUDO:
            ...
            break;
        case TIPO_ITEM_ESTRELA:
            atualizarItemEstrela( (ItemEstrela*) item->objeto, delta );
            break;
        case TIPO_ITEM_PLACAFIM:
            atualizarItemPlacaFim( (ItemPlacaFim*) item->objeto, delta );
            break;
        default:
            return;
    }
...
void desenharItem( Item *item ) {
    ...
    switch ( item->tipo ) {
        ...
        case TIPO_ITEM_ESTRELA:
            ...
            break;
        case TIPO_ITEM_PLACAFIM:
            desenharItemPlacaFim( (ItemPlacaFim*) item->objeto );
            break;
        default:
            return;
    }

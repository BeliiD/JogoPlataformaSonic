...
out_dir = "C:/Users/ninja/.gemini/antigravity/brain/93bc52cb-5abc-4480-b0d2-75a065caf794/scratch/find_shield_robust"
...
for filepath in files:
    ...
    # We define background as matching bg pixel exactly, or alpha=0
    def is_bg_pixel(p):
        if len(p) == 4 and p[3] == 0:
            return True
        return p[:3] == bg[:3]

    # Find rows with sprite pixels
    active_rows = []
    for y in range(h):
        has_sprite = any(not is_bg_pixel(img.getpixel((x, y))) for x in range(w))
        if has_sprite:
    ...
    for idx, (ys, ye) in enumerate(row_groups):
        ...
        for cidx, (xs, xe) in enumerate(col_groups):
            cw = xe - xs + 1
            print(f"    Col {cidx}: X={xs} to {xe} (width={cw})")
            if 32 <= cw <= 80 and 32 <= rh <= 80:
